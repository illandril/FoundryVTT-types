...
// biome-ignore lint/suspicious/noConfusingVoidType: There are two types of callbacks that are valid - with a return and without
type CallbackWithOptionalReturn<ReturnType> = (element: JQuery | HTMLElement) => ReturnType | undefined | void;
...
declare global {
  class Dialog<T extends string> extends Application {
    ...
    static confirm<ReturnType = void>(
      options: Pick<DialogData<'yes' | 'no'>, 'title' | 'content' | 'render'> & {
    ...
    static prompt<ReturnType = void>(
      options: Pick<DialogData<'ok'>, 'title' | 'content' | 'render'> & {
